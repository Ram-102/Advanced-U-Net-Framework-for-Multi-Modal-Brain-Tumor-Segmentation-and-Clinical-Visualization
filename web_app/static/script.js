...
function renderResults(results) {
    ...
    results.forEach(result => {
        ...
        const styleText = result.style ? result.style.toUpperCase() : 'UNKNOWN';
        const caseText = result.case || 'Unknown Case';
        const sliceText = result.slice !== undefined ? result.slice : 'N/A';
        
        resultCard.innerHTML = `
            <div class="result-header">
                <h4>${caseText} - ${styleText} (Slice ${sliceText})</h4>
                ${result.status === 'completed' ? 
                    `<button class="view-full-btn" data-image="${imageFileName}" data-title="${caseText} - ${styleText} (Slice ${sliceText})">
            ...
            <div class="result-status">
                ...
                <span style="color: #00d4ff;">Style: ${result.style || 'N/A'}</span>
            </div>
        `;
